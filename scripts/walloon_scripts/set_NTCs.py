...
def set_line_s_nom_to_ntc(n, ntc_fn):
    ...
    df["source_iso2"] = cc.convert(
        names=df["source_country_code"], src="ISO3", to="ISO2"
    )
    df["target_iso2"] = cc.convert(
        names=df["target_country_code"], src="ISO3", to="ISO2"
    )
    ...
    focus_countries = list(
        set(df["source_iso2"])
        .union(df["target_iso2"])
        .intersection(set(n.buses.country.unique()))
    )
    for pair, ntc in pair_to_ntc.items():
        ...
        lines_between = n.lines[
            (n.lines.bus0.isin(buses1) & n.lines.bus1.isin(buses2))
            | (n.lines.bus0.isin(buses2) & n.lines.bus1.isin(buses1))
        ]
        links_between = n.links[
            (n.links.carrier == "DC")
            & (
                (n.links.bus0.isin(buses1) & n.links.bus1.isin(buses2))
                | (n.links.bus0.isin(buses2) & n.links.bus1.isin(buses1))
            )
        ]
        ...
        if not links_between.empty:
            current_total_p_nom_1 = links_between.query("reversed == False")[
                "p_nom"
            ].sum()
            current_total_p_nom_2 = links_between.query("reversed == True")[
                "p_nom"
            ].sum()
        ...
        if (not updated) and (not lines_between.empty):
            ...
            if current_total_s_nom > 0:
                ...
                n.lines.loc[lines_between.index, ["s_nom", "s_nom_min"]] = ntc / len(
                    lines_between
                )
            updated = True
            line_or_link = "Line"
        if updated:
            logger.info(
                f"Set {line_or_link} capacity to a total of {ntc} MW for interconnections between {country1} and {country2}"
            )
        else:
            logger.warning(
                f"No interconnections found between {country1} and {country2}"
            )
        if removed:
            logger.info(
                f"Removed lines {removed_lines}, because there was already a valid link connection {links_between.index}."
            )
