...
def add_CCL_constraints(
    ...
    link_cfg = config.get("electricity", {}).get("agg_p_nom_limits")
    if link_cfg:
        enforce_link_limits(
            n=n,
            cfg=link_cfg,
...
def enforce_link_limits(
    n: pypsa.Network,
    cfg: dict,
    planning_horizon: str,
) -> None:
    """
    Enforce aggregated min/max capacity constraints for Link components.
    """

    link_limits = pd.read_csv(
        cfg["file"], index_col=[0, 1], header=[0, 1]
    )[planning_horizon]

    agg_carriers = cfg.get("agg_carriers", {})
    agg_buses = cfg.get("agg_buses", {})
    include_existing = cfg.get("include_existing", True)

    links = n.links.query("p_nom_extendable").rename_axis(index="Link-ext")
    if links.empty:
        return

    if agg_carriers:
        links["carrier"] = links["carrier"].replace(agg_carriers)

    def select_bus(row):
        bus_key = agg_buses.get(row["carrier"], "bus0")
        return row.get(bus_key, row["bus0"])

    def select_eff(row):
        bus_key = agg_buses.get(row["carrier"], "bus0")
        if bus_key == "bus1":
            return row.get("efficiency", 1.0) or 1.0
        if bus_key == "bus2":
            return row.get("efficiency2", 1.0) or 1.0
        return 1.0

    comp_bus = links.apply(select_bus, axis=1)
    eff = links.apply(select_eff, axis=1).replace(0, 1.0)

    grouper = pd.concat(
        [comp_bus.map(n.buses.country).rename("country"), links.carrier], axis=1
    )
    lhs = n.model["Link-p_nom"].groupby(grouper).sum()

    eff_group = eff.groupby([grouper.country, grouper.carrier]).mean()
    idx = link_limits.index.intersection(eff_group.index)
    link_limits.loc[idx] = link_limits.loc[idx].div(eff_group.loc[idx], axis=0)

    if include_existing:
        links_cst = n.links.query("~p_nom_extendable").rename_axis(index="Link-cst")
        links_cst = links_cst[
            (links_cst["build_year"] + links_cst["lifetime"]) >= int(planning_horizon)
        ]
        if agg_carriers:
            links_cst["carrier"] = links_cst["carrier"].replace(agg_carriers)
        comp_bus_cst = links_cst.apply(select_bus, axis=1)
        rhs_cst = (
            pd.concat(
                [
                    comp_bus_cst.map(n.buses.country).rename("country"),
                    links_cst[["carrier", "p_nom"]],
                ],
                axis=1,
            )
        ...
        rhs_min = link_limits["min"].dropna()
        idx_min = rhs_min.index.join(rhs_cst.index, how="left")
        rhs_min = rhs_min.reindex(idx_min).fillna(0)
        rhs = (rhs_min - rhs_cst.reindex(idx_min).fillna(0).p_nom).dropna()
        rhs[rhs < 0] = 0
        ...
        minimum = xr.DataArray(link_limits["min"].dropna()).rename(dim_0="group")

    idx_min = minimum.indexes["group"].intersection(lhs.indexes["group"])
    if not idx_min.empty:
        n.model.add_constraints(
            lhs.sel(group=idx_min) >= minimum.loc[idx_min],
            name="agg_p_nom_min-Link",
        )

    if include_existing:
        rhs_max = link_limits["max"].dropna()
        idx_max = rhs_max.index.join(rhs_cst.index, how="left")
        rhs_max = rhs_max.reindex(idx_max).fillna(0)
        rhs = (rhs_max - rhs_cst.reindex(idx_max).fillna(0).p_nom).dropna()
        rhs[rhs < 0] = 0
        ...
        maximum = xr.DataArray(link_limits["max"].dropna()).rename(dim_0="group")

    idx_max = maximum.indexes["group"].intersection(lhs.indexes["group"])
    if not idx_max.empty:
        n.model.add_constraints(
            lhs.sel(group=idx_max) <= maximum.loc[idx_max],
            name="agg_p_nom_max-Link",
        )
